...
class HIVTreatmentWrapper(object):
    def __init__(self, param_set = None):
        self.param_set = param_set
        self.env = HIVTreatment()
        self.reset()
    ...
    def reset(self):
        if self.param_set is None:
            ...
            return self.env.reset(perturb_params = True, **self.param_set)
    
    def step(self, action):
        if self.param_set is None:
            reward_unscaled, next_state = self.env.perform_action(action)
        else:
            reward_unscaled, next_state = self.env.perform_action(action, perturb_params=True, **self.param_set)
        terminate = self.env.is_done()
        return next_state, reward_unscaled, terminate, None
